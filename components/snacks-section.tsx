...
export default function SnacksSection() {
  const snacksProducts = allProducts.filter((p) => p.category === "snacks")
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {snacksProducts.map((product) => (
            ...
            >
              ...
              {/* Product Info */}
              <div className="p-4">
                <h3 className="font-serif text-amber-900 text-lg mb-2 line-clamp-2">{product.name}</h3>
                    ...
                    {[...Array(5)].map((_, i) => (
                      <svg
                        ...
                        className={`w-3.5 h-3.5 ${
                          i < Math.floor(product.rating || 0) ? "fill-amber-400 text-amber-400" : "text-gray-300"
                        }`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
